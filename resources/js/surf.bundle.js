      ...
      13695: (e, t, s) => {
        ...
        class te {
          ...
          getCurrentScoreFormatted() { // send score
            ...
            switch (this.session.settings.mode) {
              case q.Endless:
              case q.ZigZag:
                return W(this.game.time.elapsed).toString() + ", \n" + Math.floor(this.game.dist.unit).toString();
                //  return e.toString();
              case q.TimeTrial:
                return W(e);
            }
        ...
        buildCluster(type, x, y, clusterId) {
          ...
          buildObject(e, t, s, i) {
            ...
            switch (
              (a || (a = this.createObject(e, t, s, i)), a.type) // level gen
            ) {
              case "wall":
                !["f1", "f2", "f3"].includes(i) &&
                  Math.random() < 0.5 &&
        ...
        class Me {
          ...
          checkJoysticks(gamepad) {
            const threshold = 0.5;
            const right =
              gamepad.axes[0] > threshold || gamepad.axes[2] > threshold; // right
            const left =
              gamepad.axes[0] < -threshold || gamepad.axes[2] < -threshold; // left
          ...
          checkButtons(gamepad) {
            const isPressed = (button) =>
              typeof button === "object" ? button.pressed : button === 1;
            ...
            for (let i = 0; i < totalButtons; i++) {
              if (isPressed(gamepad.buttons[i])) {
                ...
                if (
                  ...
                  ["a", "b", "up", "down"].includes(this.buttonMap[i])
                ) {
                ...
                switch (bmi) {
                  ...
                  case "home":
                  case "touchpad":
                    Ae.sys.routeInput(He.Settings);
                    break;
                }

                console.log("Button pressed: " + bmi);

                Ae.sys.checkCheatCode(bmi);
              } else {
